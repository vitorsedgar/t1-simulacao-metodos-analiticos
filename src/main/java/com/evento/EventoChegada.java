...
public class EventoChegada extends EventoAbstract {
  ...
  public EventoChegada(double tempo, String filaOrigem) {
  ...
  public void executa(List<Fila> filas) {
    ...
    Fila fila = filas.stream().filter(fila1 -> indexFilaOrigem.equals(fila1.nome)).findFirst()
        .get();
      ...
      if (fila.possuiServidorDisponivel()) {
        ...
        if (destinoRoteamento != null) {
          Escalonador.agendar(new EventoRoteamento(
              Contexto.tempoGlobal + GeradorNumeroAleatorio
                  .getNextEventTime(fila.getTempoSaidaMinimo(), fila.getTempoSaidaMaximo()),
              indexFilaOrigem,
              destinoRoteamento)
          );
        } else {
    ...
    Escalonador.agendar(new EventoChegada(
        Contexto.tempoGlobal + GeradorNumeroAleatorio
            .getNextEventTime(fila.getTempoChegadaMinimo(), fila.getTempoChegadaMaximo()),
        indexFilaOrigem)
    );
  }
